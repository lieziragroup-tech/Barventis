...
export default function CostControl({ stock, transactions, invoices }) {
  ...
  const handleExportExcel = () => {
    ...
    const dailyData = dailyColumns.map(row => ({
      ...
      'Sales (IDR)': row.sales,
      'Ratio %': row.sales > 0 ? ((row.purchase / row.sales) * 100).toFixed(1) : '0.0'
    }));
  ...
  const handlePrintPDF = () => {
    const printHTML = `
      ...
      <table>
        <thead><tr><th>Date</th><th style="text-align:right">Purchases</th><th style="text-align:right">Sales</th><th style="text-align:right">Ratio</th></tr></thead>
